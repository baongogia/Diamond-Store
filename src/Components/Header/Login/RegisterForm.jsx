...
export default function RegisterForm() {
  ...
  const validate = async () => {
    ...
    try {
      const checkPhone = await axios.post(
        `https://diamondstoreapi.azurewebsites.net/api/Accounts/CheckUsernameExist?username=${formData.Username}`
      );
      if (checkPhone.data) {
    ...
    try {
      const checkPhone = await axios.post(
        `https://diamondstoreapi.azurewebsites.net/api/Accounts/CheckEmailExist?email=${formData.Email}`
      );
      if (checkPhone.data) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Registering the user in your backend
      ...
      // Reloading the page after successful registration
  ...
  return (
    <div className="relative font-serif flex items-center flex-col w-full h-[130vh]">
